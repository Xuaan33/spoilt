...
class _HistoryState extends State<History> {
  ...
  String _extractPrediction(String predictionText) {
    ...
    if (predictionLine.isNotEmpty) {
      final prediction = predictionLine.split(':').last.trim();
      final allergies = widget.userData?['allergies'];

      if (allergies != null &&
          allergies
              .toString()
              .toLowerCase()
              .contains(prediction.toLowerCase())) {
          ...
          : RefreshIndicator(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Dismissible(
                    key: Key(
                        data['timestamp']), // Use a unique key for each item
                    ...
                    child: _buildHistoryItem(data, timestamp),
                  );
                },
              ),
            ),
  ...
  Widget _buildHistoryItem(Map<String, dynamic> data, DateTime timestamp) {
    return Card(
      child: ListTile(
        title: Column(
              ...
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Spot it ${timeago.format(timestamp)}'),
                    if (data['prediction']
                        .toString()
                        .toLowerCase()
                        .contains('fresh'))
                    ...
                    if (data['prediction'] != null &&
                        widget.userData?['allergies'] != null &&
                        widget.userData?['allergies'].any((allergy) =>
                            _extractPrediction(data['prediction'] ?? '')
                                .contains(allergy)))
                      Text(
                        'Allergic status: Yes',
